...
@Component({
  selector: 'app-salariee',
  templateUrl: './salariee.component.html',
  styleUrls: ['./salariee.component.css']
})
export class SalarieeComponent {
  ...
  constructor(private router:Router,private route:ActivatedRoute,private compteTiersService: CompteTiersService,private confirmationDialogService: ConfirmationDialogService) {
    ...
    this.compteTiersService.loadSalariee(this.idSociete).subscribe(salariee => {
      this.dataSource=salariee;
  ...
  remove(id){
    ...
    this.confirmationDialogService.openConfirmationDialog(confirmationData)
      .then(result => {
        if (result) {
          this.compteTiersService.supprimerSalariee(id)
          location.reload();
        } else {
