...
@Component({
  selector: 'app-details-code-journal',
  templateUrl: './details-code-journal.component.html',
  styleUrls: ['./details-code-journal.component.css']
})
export class DetailsCodeJournalComponent {
  idSociete: any;
  idExercice: any;
  displayedColumns: string[] = ['position', 'periode', 'code', 'intitule_journale', 'action'];
  ...
  constructor(private http: HttpClient, private route: ActivatedRoute, private exerciceService: ExerciceService, private router: Router, private confirmationDialogService: ConfirmationDialogService, private attentionDialogService: AttentionDialogService) {
    ...
    this.exerciceService.loadSaisiJournal(this.idExercice).subscribe(exercice => {
  ...
  filterItems() {
    ...
    this.dataSourceBackup = new MatTableDataSource<any>(
      this.dataSource.filter(user =>
        user.intitule_journale.toLowerCase().includes(this.inputElement.value.toLowerCase()) || user.periode.toLowerCase().includes(this.inputElement.value.toLowerCase())
      )
    )
